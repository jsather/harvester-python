...
"""
# TODO: First try 2 gpu and see if cost effective, then consider using 
#       darkflow and running all networks on same graph
...
# from: https://stackoverflow.com/questions/37893755/tensorflow-set-cuda-visible-devices-within-jupyter 
# os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"   # see issue #152
os.environ["CUDA_VISIBLE_DEVICES"]="0"
import tensorflow as tf

import ddpg_config as ddpg_cfg
import networks_config as net_cfg
from agent import agent_config as agent_cfg 
from agent.agent_ros import HemiAgentROS 
...
def get_latest(weights_dir):
    ...
    while dir_contents:
        ...
        weights_file = latest + '/ddpg*'
        vars_file = latest + '/training_vars.pkl'

        if glob.glob(weights_file) and glob.glob(vars_file):
            good_pickle = True
            ...
            if good_pickle:
                return weights_file[:-1], vars_file
...
class DDPG(object):
    ...
    def __init__(self, session, actor, critic):
        ...
        # Initialize agent
        self.agent = HemiAgentROS(headless=ddpg_cfg.headless)
    ...
    def build_summaries(self):
        ...
        summary_vars = [episode_reward, episode_ave_max_q, episode_steps]
    ...
    def log_episode_data(self, reward, ep_ave_max_q, ep_train_iter, ep, steps):
        ...
        summary_str = self.session.run(
            ...
            feed_dict={
                ...
                self.summary_vars[1]: q, 
                self.summary_vars[2]: steps})
    ...
    # def step_and_train(self, action):
    #     """ Sends message to agent to take step and trains networks during dead
    #         time (one batch at a time)
    #     """
    #     att = 0
    #     train_iter = 0
    #     sum_ave_max_q = 0.
    #     elapsed = 0.
    #     self.agent.clear_step_return()

    #     while att < self.agent.max_attempts:
    #         ret = self.agent.get_step_return()
    #         if ret:
    #             self.agent.clear_step_return()
    #             self.agent.inc_step_id()
    #             return ret, sum_ave_max_q, train_iter
    #         elif att == 0 or elapsed > self.agent.timeout:
    #             self.agent.publish_action(action)
    #             start = time.time()
    #             att += 1
    #         else:
    #             time.sleep(0.1)
    ...
    #     print('No feedback received. Stopping process.')
    #     os.kill(os.getpid(), signal.SIGTERM)
    
    def train_in_parallel(self, process, args=(), timeout=10):
        """ Runs process (reset or step) and trains in parallel using
            multiprocessing. Returns state, sum_average_max_q and train 
            iters. 
        """
        proc = multiprocessing.Process(target=process, args=args)
        proc.start() 
        
        sum_ave_max_q = 0. 
        train_iter = 0 
        start = time.time() 
        print("DEBUG: Fake training in parallel with process: " + str(process))
        sys.stdout.flush()

        while proc.is_alive():
            if time.time() - start > timeout: 
                raise Exception(
                    'Timeout error: no response from agent during ' + 
                    str(process))
            sum_ave_max_q += 1.0 #self.train_networks(update_steps=1) 
            train_iter += 1 
            time.sleep(1.0)
            print('Train iter' + str(train_iter))

        print("DEBUG: Completed " + str(train_iter) + " steps")
        sys.stdout.flush()

        return sum_ave_max_q, train_iter 

    def step_and_train(self, action, timeout=10):
        """ Takes step in environment and trains in parallel. """
        sum_ave_max_q, iter = self.train_in_parallel(self.agent.step, 
            args=(action), timeout=timeout)
        ret = self.agent.get_last_return()

        return ret, sum_ave_max_q, iter 

    def reset_and_train(self, timeout=20):
        """ Resets in environment and trains in parallel. """
        sum_ave_max_q, iter = self.train_in_parallel(self.agent.reset,
            timeout=timeout)
    ...
    def learn(self):
        ...
        for ep in range(start, self.max_episodes):
            ...
            for step in range(self.max_ep_steps):
                ...
                # Sample from buffer and train between each step
                if self.total_steps < self.pretrain_steps:
                    [o2, j2], r, t, _ = self.agent.step(a)
                else:
                    [[o2, j2], r, t, _], q, iter = self.step_and_train(a)
                    ep_train_iter += iter
                    ep_ave_max_q += q

                step_elapsed = time.time() - step_start
                print("DEBUG: step time: " + str(step_elapsed) + "s")
                sys.stdout.flush()
...
def main():
    ...
    set_logfile()
    config = tf.ConfigProto(**net_cfg.tf_cfg)
